...
import { FileSearch2, GitCommit, Github, Star } from "lucide-react";
  ...
  return (
    <>
      <div className="flex max-md:flex-col gap-8 md:gap-16">
        ...
        <span className="md:hidden text-gray-300 text-sm">{user?.bio}</span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 gap-4 w-full">
          ...
          <DataSection title="Total Followers" value={user?.followers || 0}>
      ...
      <div className="my-8">
        <p className="max-md:hidden text-gray-300">{user?.bio}</p>
      </div>
      {!user && (
        <div className="md:p-4 p-3 max-md:text-xs rounded-lg border border-orange-500 bg-orange-700/20">
          <p>Submit you Github Access Token to see profile stats</p>
        </div>
      )}
      {session?.user.subscription === "free" ||
        (session?.user?.subscription === null &&
          session?.user.total_commits &&
          session?.user.total_commits > 10 && (
            <div className="md:p-4 p-3 max-md:text-xs rounded-lg border border-orange-500 bg-orange-700/20 flex gap-3">
              <FileSearch2 className="text-orange-500" />
              <p>
                You have exceeded the free limit of total commits. Upgrade your
  ...
  return (
    <div className="bg-gradient-to-tr from-blue-900/20 via-indigo-900/20 to-purple-900/20 bg-opacity-50 p-4 md:p-8 rounded-2xl border border-gray-700">
      <div className="flex items-center justify-between">
        {children}
        <div className="md:hidden text-gray-400 mt-2">{title}</div>
        <span className="text-2xl md:text-4xl text-gray-200">{value}</span>
      </div>
      <div className="max-md:hidden text-gray-400 mt-2">{title}</div>
    </div>
  );
};
